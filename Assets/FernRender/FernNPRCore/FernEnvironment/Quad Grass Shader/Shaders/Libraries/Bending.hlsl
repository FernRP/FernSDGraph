...
BendData GetBendDatas(uint mode, float mask, float pushStrength, float flattenStrength, float perspCorrection)
{
	BendData s = (BendData)0;
...
float4 GetBendOffset(float3 wPos, BendData b)
{
	float4 offset = 0;
...
void GetBendOffset_float(float3 wPos, float mask, float pushStrength, float flattenStrength, out float4 offset)
{
	...
	BendData b = GetBendDatas(0, mask, pushStrength, flattenStrength, 0);
