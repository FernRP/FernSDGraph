...
WindData GetWindDatas(in float strength, float speed, float4 direction, float swinging, float mask, float randObject, float randVertex, float randObjectStrength, float gustStrength, float gustFrequency, float gustSpeed)
{
	WindData windSettings = (WindData)0;
...
float2 GetGustingUV(float3 positionWS, WindData windSettings)
{
...
float SampleGustMap(float3 positionWS, WindData windSettings)
{
...
float4 GetWindOffset(in float3 positionOS, in float3 positionWS, float rand, WindData windSettings)
{
	float4 offset = 0;
...
{
	WindData windSettings = GetWindDatas(strength, speed, direction.xyzz, swinging, mask, randObject, randVertex, randObjectStrength, gustStrength, gustFrequency, gustSpeed);
