...
void AlphaClip(float alpha, float cutoff, float3 clipPos, float3 wPos, float4 fadeParamsNear, float4 fadeParamsFar,
...
VertexInputs GetVertexInputs(Attributes v, float upNormals)
{
    VertexInputs i = (VertexInputs)0;
    i.positionOS = v.positionOS;
    i.normalOS = lerp(v.normalOS, float3(0, 1, 0), upNormals);
    #if defined(_NORMALMAP) || defined(CURVEDWORLD_NORMAL_TRANSFORMATION_ON)
...
VertexOutput GetVertexOutput(VertexInputs input, float rand, WindData s, BendData b)
{
