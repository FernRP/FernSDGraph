...
Varyings LitPassVertex(Attributes input)
{
	...
	float posOffset = PosOSToRand01();

	WindData wind = GetWindDatas(_WindAmbientStrength, _WindSpeed, _WindDirection, _WindSwinging, BEND_MASK, _WindObjectRand, _WindVertexRand, _WindRandStrength, _WindGustStrength, _WindGustFreq, _WindGustSpeed);
	BendData bending = GetBendDatas(_BendMode, BEND_MASK, _BendPushStrength, _BendFlattenStrength, _PerspectiveCorrection);

	VertexInputs vertexInputs = GetVertexInputs(input, _UpNormal);
...
#endif
{
	...
	WindData wind;
	if(_WindGustTint > 0) wind = GetWindDatas(_WindAmbientStrength, _WindSpeed, _WindDirection, _WindSwinging, AO_MASK, _WindObjectRand, _WindVertexRand, _WindRandStrength, _WindGustStrength, _WindGustFreq, _WindGustSpeed);
