...
{	
	Properties
	{
		...
		[Tooltip(Select which channel from the R input texture to write in the R output channel)]_CombineModeR("Output R", Float) = 0
		[Tooltip(Select which channel from the G input texture to write in the G output channel)]_CombineModeG("Output G", Float) = 1
		[Tooltip(Select which channel from the B input texture to write in the B output channel)]_CombineModeB("Output B", Float) = 2
		[Tooltip(Select which channel from the A input texture to write in the A output channel)]_CombineModeA("Output A", Float) = 3
	...
	SubShader
	{
		...
		{
			HLSLPROGRAM
			#include "../../Shaders/SDMixFixed.hlsl"
