...
namespace FernNPRCore.SDNodeGraph
{
    ...
    public class SDPromptRegisterNodeView : SDNodeView
    {
        ...
        public SDPromptRegisterNodeView()
        {
            ...
            promptDataRects = new List<PromptDrawData>();
        }
        ...
        private SDPromptFavoriteData favorite_Asset;
        ...
        public IEnumerator SearchWords(int menusIndex, string str, List<List<string>> config)
        {
            searchwords.Clear();
            ...
            if (menusIndex == 0 || menusIndex > config.Count)
            {
                foreach (var line in config)
                {
                    for (var i = 1; i < line.Count; i++)
                    {
                        ...
                        str = str.ToLower();
                        var keywords = str.Split(' ', ',', ';', '|', '*', '&');// Some possible separators
        ...
        public static bool DrawSearchField(ref string searchingText, GUIContent[] searchModeMenus, int selectedMenusIndex = 0, Action<object, string[], int> menuCallback = null) // 
        {
            ...
            if (string.IsNullOrEmpty(searchingText) && Event.current.type == EventType.Repaint)
            {
                ...
                {
                    Rect position1 = toolbarSearchTextFieldPopup.padding.Remove(new Rect(rect.x, rect.y, rect.width, toolbarSearchTextFieldPopup.fixedHeight > 0.0 ? toolbarSearchTextFieldPopup.fixedHeight : rect.height));
        ...
        void OnGUI()
        {
            ...
            // ----------------------------------------------DrawSearchField---------------------------------------
            EditorGUILayout.BeginHorizontal();
            var DoSearch = DrawSearchField(ref searchingText, modeMenus, menusIndex, (_, _, arg3) =>
            {
                SetMenusIndex(arg3);
            });
            ...
            var title_t = EditorGUI.TextArea(controlRect, registerRegisterData.title.tooltip);
            // display search mode
            if (string.IsNullOrEmpty(title_t) && Event.current.type == EventType.Repaint)
            {
                using (new EditorGUI.DisabledScope(true))
                {
                    EditorStyles.label.Draw(controlRect, "Untitled", false, false, false, false);
                }
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                promptDatas = toolBarIndex == 0 ? registerRegisterData.positiveDatas : registerRegisterData.negativeDatas;
                refresh = true;
            }
            DrawSplitter();
            s2 = EditorGUILayout.BeginScrollView(s2, GUILayout.Height(230));
            DrawPromptWords(current_idx, 0, refresh, promptDatas);
        ...
        private List<PromptDrawData> promptDataRects;
        ...
        private void DrawPromptWords(int currentIdx, int startIndex, bool refresh, List<PromptData> promptDatas)
        {
            ...
            {
                var promptData = promptDatas[i];
                PromptDrawData drawData;
                var btn_rect = new Rect();
                bool drawNext = false;
                var label = new GUIContent();
                if (refresh)
                {
                    if (i == 0) promptDataRects.Clear();
                    var weight = promptData.weight;
                    var realWord = TryGetENWord(promptData.word) + (weight > 0 ? $"[{weight}]":"");
                    var showWord = realWord;
                    GetShowWord(ref showWord, 12);
                    label.text = showWord;
                    label.tooltip = realWord;
                    drawData.size = GUI.skin.button.CalcSize(label);
                    drawData.label = label;
                    
                    wordWidth += drawData.size.x + 4;
                    drawNext = wordWidth > 250;
                    if (!drawNext)
                    {
                        btn_rect = GUILayoutUtility.GetRect(drawData.size.x, drawData.size.y + 4);
                        drawData.rect = btn_rect;
                        promptDataRects.Add(drawData);
                    }
                }
                else
                {
                    drawData = promptDataRects[i];
                    wordWidth += drawData.size.x + 4;
                    drawNext = wordWidth > 250;
                    label = drawData.label;
                    if (!drawNext)
                    {
                        btn_rect = GUILayoutUtility.GetRect(drawData.size.x, drawData.size.y + 4);
                        drawData.rect = btn_rect;
                        promptDataRects[i] = drawData;
                    }
                }
                ...
                GenerateDrag(btn_rect,
                    ...
                    onDragExited:_=>
                    {
                        if (selected_idx != insert_idx && insert_idx != -1 && selected_idx != -1)
                        {
                            ...
                            var data = promptDatas[selected_idx];
                            promptDatas.RemoveAt(selected_idx);
                            promptDatas.Insert(insert_idx, data);
                ...
                {
                    if (i == insert_idx)
                        label = promptDataRects[selected_idx].label;
                    if (i > insert_idx && i <= selected_idx)
                        label = promptDataRects[i - 1].label;
                }
            ...
            {
                DrawPromptWords(currentIdx, nextIndex, refresh, promptDatas);
            }
            else
            {
                if (selected_idx != -1)
                {
                    ...
                    {
                        if (!promptDataRects[i].rect.Contains(position)) continue;
                        idx = i;
                        break;
                    }
        ...
        public static void DrawSplitter(bool isBoxed = false, bool isFullWidth = false, float width = 1f, float height = 1f)
        {
            ...
            if (isBoxed)
            {
                rect.xMin = Math.Abs(xMin - 7.0) < 0.1f ? 4.0f : EditorGUIUtility.singleLineHeight;
                rect.width -= 1;
            }
