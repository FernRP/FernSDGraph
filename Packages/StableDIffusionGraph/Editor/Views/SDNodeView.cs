...
namespace UnityEditor.SDGraph
{
	...
	public class SDNodeView : BaseNodeView
	{
		...
		void CreateTexturePreviewImGUI(VisualElement previewContainer, SDNode node)
		{
			...
			SDEditorUtils.ScheduleAutoHide(previewContainer, owner);
		...
		void DrawTextureInfoHover(Rect previewRect, Texture texture)
		{
			...
			if (!(EditorWindow.mouseOverWindow is StableDiffusionGraphWindow sdGraphWindow && sdGraphWindow.GetCurrentGraph() == owner.graph))
				return;
