Shader "Hidden/SDMix/RidgedPerlinNoise"
{	
	Properties
	{
		...
		[ShowInInspector]_OutputRange("Output Range", Vector) = (0, 1, 0, 0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "Packages/com.tateam.sdgraph/Shaders/PerlinNoise.hlsl"
...
#ifdef CRT_2D
				if (_UVMode == 0)
					noise = GenerateRidgedPerlin2DNoise(uvs.xy, _Frequency, _Octaves, _Persistance, _Lacunarity, seed);
				else
#endif
					noise = GenerateRidgedPerlin3DNoise(uvs, _Frequency, _Octaves, _Persistance, _Lacunarity, seed).x;
